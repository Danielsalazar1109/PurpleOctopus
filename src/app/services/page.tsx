...
export default function Services() {
  ...
  const slides = [
    ...
    {
      type: "image",
      src: "/images/condo.jpeg",
      link: "/condo",
      title: "CONDO CLEANING",
    },
    {
      type: "image",
      src: "/images/moving.jpeg",
      link: "/moving",
      ...
      <div className="relative w-full max-w-7xl overflow-hidden" ref={emblaRef}>
        <div className="flex">
          {slides.map((slide, index) => (
            <div 
              key={index} 
              className="flex-[0_0_100%] sm:flex-[0_0_50%] md:flex-[0_0_33%] lg:flex-[0_0_33%] mx-2 p-4 sm:p-5 md:p-6 bg-white text-black rounded-xl shadow-lg hover:shadow-xl transition-shadow duration-300 text-center transform hover:-translate-y-1 transition-transform"
            >
              <Link href={slide.link ?? '#'} className="block h-full">
                ...
                <button className="bg-purple-600 hover:bg-purple-700 text-white py-2 px-4 rounded-lg transition-colors duration-300">LEARN MORE</button> 
                </Link>
                </div>    
              </Link>
            </div>
